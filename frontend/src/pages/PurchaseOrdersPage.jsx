...
Falls back to regex for header fields.
"""
...
def _clean_cell(val) -> str:
    """Remove embedded newlines from PDF cell values like '10,000.0\\n0'."""
    if val is None:
        return ""
    return re.sub(r'\n', '', str(val)).strip()
...
def _parse_line_items_from_table(tables: list) -> list:
    """
    Find the line items table (has Sr./Item/Qty/Rate columns) and parse it.
    """
    items = []
    for table in tables:
        ...
        for row in table[1:]:
            if not row:
                continue
            # Skip summary rows (Total, Net Amount, etc.)
