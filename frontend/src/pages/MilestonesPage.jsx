...
export default function MilestonesPage() {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      ...
      const msg = Array.isArray(detail) ? detail.map(d => d.msg).join(', ') : detail || 'Failed';
      toast.error(msg);
    }
  };
  ...
  return (
    <div className="p-8">
      ...
      <div className="rounded-xl border border-border bg-white shadow-sm overflow-hidden">
        {milestones.length === 0 ? (
          <p className="px-6 py-12 text-center text-sm text-[#9CA3AF]">No milestones</p>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-gray-50 text-left text-xs font-medium text-[#6B7280]">
                {['Title', 'Invoice', 'Amount', 'Due Date', 'Status'].map(h => (
            ...
            <tbody>
              {milestones.map(m => (
                <tr key={m.id} className="border-b border-border last:border-0 hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3 text-[#6B7280]">{m.end_date || m.due_date || '—'}</td>
      ...
      {showModal && (
        <Modal title="New Milestone" onClose={() => setShowModal(false)}>
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <Input label="Title" value={form.title} onChange={set('title')} required placeholder="e.g. Final delivery" />
